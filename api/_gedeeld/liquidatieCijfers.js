 ...
 *
 * LET OP: `src/medewerker/liquidatieCijfers.js` is de spiegel van dit bestand voor de browser
 * (ESM i.p.v. CommonJS). Wijzig je hier een regel of een formule, wijzig 'm daar dan mee.
 */
...
const ALLE_REGELS = [...BALANS_ACTIVA, ...BALANS_PASSIVA, ...RESULTAAT];
/** Alleen wat je zelf intikt — de rest rekenen we uit. */
const INVULSLEUTELS = ALLE_REGELS.filter((r) => !r.berekend).map((r) => r.sleutel);
const TOTAALSLEUTELS = ALLE_REGELS.filter((r) => r.berekend).map((r) => r.sleutel);
 ...
 */
function balansVerschil(cijfers) {
