...
const { zetAanvraag } = require("../_gedeeld/rittenInstellingen");

/**
 * POST /api/ritten-aanvraag  body { accountId }
 *
 * Voor een klant die de Rittenregistratie voor één van zijn gekoppelde accounts nog niet heeft
 * (rittenIngeschakeld = false, zie mijn-gegevens) en op "Vraag aan" klikt. Zet géén toegang aan —
 * legt alleen vast wie wanneer heeft aangevraagd, zodat de beheerder dit terugziet in
 * Beheer → Ritten en het (na het regelen van de betaling) zelf kan aanzetten.
 *
 * Zelfde opzet als api/uren-aanvraag: bewust GEEN rittenToegang-check (die vereist juist dat de
 * module al aan staat) — alleen de "hoort dit account bij de ingelogde gebruiker"-check.
 */
