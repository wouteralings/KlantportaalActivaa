...
import { Boxes, CheckCircle2, ChevronDown, Clock, Download, Lock, Package, PackageX, Search } from "lucide-react";
...
function BezittingenUitlegBanner() {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12.5, color: KLEUR.subtekst }}>
        <strong style={{ color: KLEUR.tekst }}>Bezittingen is beschikbaar voor deze administraties.</strong>{" "}
...
export default function BezittingenModule({ accounts, prijs = 5, alleenLezen = false }) {
  ...
  if (accounts.length === 1) {
    ...
    return acc.bezittingenIngeschakeld
      ? <BezittingenInhoud account={acc} alleenLezen={alleenLezen} />
      : <BezittingenNietActief account={acc} prijs={prijs} />;
  }
  ...
  const actieveAccounts = lijst.filter((a) => a.bezittingenIngeschakeld);
  const nietActieveAccounts = lijst.filter((a) => !a.bezittingenIngeschakeld);
