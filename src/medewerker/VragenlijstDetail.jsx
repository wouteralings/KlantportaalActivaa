              ...
              return (
                <div key={d.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, fontSize: 12.5, padding: "6px 9px", border: `1px solid ${klaar ? "#BFE0C8" : KLEUR.rand}`, borderRadius: 7, background: klaar ? "#F1F8F3" : "#fff" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div><span style={{ fontWeight: 600 }}>{d.naam}</span>{d.verplicht === false && <span style={{ color: KLEUR.mutedTekst }}> · optioneel</span>}{isVraagType && <span style={{ fontSize: 10.5, fontWeight: 700, marginLeft: 6, padding: "1px 6px", borderRadius: 999, background: KLEUR.lichtblauw, color: KLEUR.blauw }}>{VRAAGTYPE_LABEL[d.type] || "Vraag"}</span>}{d.vanToepassing === false && <span style={{ fontSize: 10.5, fontWeight: 700, marginLeft: 6, padding: "1px 6px", borderRadius: 999, background: "#F0F0EC", color: KLEUR.mutedTekst }}>overgeslagen</span>}</div>
                    {d.vanToepassing === false && <div style={{ fontSize: 11.5, color: KLEUR.mutedTekst, fontStyle: "italic" }}>Voorwaardelijk — nu niet van toepassing (telt niet mee, wordt niet weggeschreven).</div>}
                    {isVraagType && (
