...
/**
 * Dividendlogboek — medewerkersportaal → Klantoverzicht → Dividenduitkeringen → tabblad "Logboek".
 *
 * Losse tegenhanger van NotulenLogboek.jsx. Eén centraal overzicht van álle in het portaal
 * opgestelde dividendstukken over alle cliënten (uit /api/medewerker-dividend-opslaan zonder
 * accountId): datum, cliënt, model, wie het opstelde en een snellink naar het stuk in SharePoint.
 * Openen zet het stuk terug in "Dividendstuk opstellen"; een beheerder kan een regel uit het logboek
 * halen (het stuk en het dividenddossier blijven staan).
 *
 * Het dossieroverzicht van Dividenduitkeringen blijft daarnaast bestaan — dat gaat over de dossiers
 * in Dynamics. Dit logboek gaat over de stukken die in het portaal zijn opgemaakt.
 */
const KLEUR = {
...
export default function DividendLogboek({ onNieuwStuk, onBewerken, isBeheerder = false, magVerwijderen = null }) {
  ...
  const [stukken, setStukken] = useState(null); // null = laden
  ...
  async function laad() {
    ...
    try {
      ...
      setStukken(Array.isArray(d.stukken) ? d.stukken : []);
    } catch (e) {
      setStukken([]); setFout(String((e && e.message) || e));
    } finally {
      setBezig(false);
    }
  }
  ...
  async function verwijderRegel(n) {
    const naam = `${veiligeStr(n.modelNaam) || "Dividenduitkering"} — ${veiligeStr(n.klantnaam)}`;
    if (typeof window !== "undefined" && !window.confirm(`"${naam}" verwijderen?\n\nDe regel verdwijnt uit het logboek ÉN het bestand wordt uit de SharePoint-map van de cliënt verwijderd. Hoort er een aangifte dividendbelasting bij, dan gaat die ook weg.\n\nHet dividenddossier in Dynamics blijft staan — dat is de administratie.\n\nDit kan niet ongedaan gemaakt worden.`)) return;
    ...
    try {
      ...
      setStukken((lijst) => (lijst || []).filter((x) => x.dossierId !== n.dossierId));
    ...
    <div style={{ maxWidth: 1600, margin: "0 auto", padding: "0 24px 40px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
        ...
        {onNieuwStuk && (
          ...
          >
            <Plus size={14} /> Nieuw dividendstuk
          </button>
        )}
      </div>
      ...
      {stukken === null ? (
        <div style={{ fontSize: 13, color: KLEUR.mutedTekst, padding: "16px 0" }}>Dividendlogboek laden…</div>
      ) : totaal === 0 ? (
        <div style={{ fontSize: 13, color: KLEUR.mutedTekst, padding: "16px 0" }}>
          {zoek.trim() ? "Geen dividendstukken gevonden met deze zoekterm." : "Er zijn nog geen dividendstukken opgesteld in het portaal."}
        </div>
      ) : (
        <>
                ...
                {zichtbaar.map((n) => (
                  <tr key={n.dossierId}>
                    ...
                    <td style={{ ...td, textAlign: "right", whiteSpace: "nowrap" }}>
                      ...
                      {onBewerken && (
                        <button onClick={() => onBewerken(n)} title="Openen in Dividendstuk opstellen" style={{ ...knopLicht, marginLeft: 6 }}>
