 ...
 *
 * Eén centraal overzicht van álle opgestelde liquidatiestukken over alle cliënten (uit
 * /api/medewerker-liquidatie-opslaan zonder accountId): datum, cliënt, model, wie het opstelde en een
 ...
 * uit het logboek halen; het liquidatiedossier in Dynamics blijft staan.
 *
 * Het Dynamics-dossieroverzicht blijft daarnaast bestaan: dáár staat de status en de behandelaar van
 * elk liquidatiedossier. Dit logboek gaat over de stukken die in het portaal zijn opgemaakt.
 */
const KLEUR = {
...
export default function LiquidatieLogboek({ onNieuwStuk, onBewerken, isBeheerder = false, magVerwijderen = null }) {
  ...
  async function laad() {
    ...
    try {
      ...
      setStukken(Array.isArray(d.liquidatie) ? d.liquidatie : []);
    } catch (e) {
    ...
    <div style={{ maxWidth: 1600, margin: "0 auto", padding: "0 24px 40px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
        ...
        {onNieuwStuk && (
          ...
          >
            <Plus size={14} /> Nieuw liquidatiestuk
          </button>
        )}
      </div>
      ...
      {stukken === null ? (
        <div style={{ fontSize: 13, color: KLEUR.mutedTekst, padding: "16px 0" }}>Liquidatielogboek laden…</div>
      ) : totaal === 0 ? (
        <div style={{ fontSize: 13, color: KLEUR.mutedTekst, padding: "16px 0" }}>
          {zoek.trim() ? "Geen liquidatiestukken gevonden met deze zoekterm." : "Er zijn nog geen liquidatiestukken opgesteld in het portaal."}
        </div>
      ) : (
        <>
                ...
                {zichtbaar.map((n) => (
                  <tr key={n.dossierId}>
                    ...
                    <td style={{ ...td, textAlign: "right", whiteSpace: "nowrap" }}>
                      ...
                      {onBewerken && (
                        <button onClick={() => onBewerken(n)} title="Openen in Liquidatiestuk opstellen" style={{ ...knopLicht, marginLeft: 6 }}>
