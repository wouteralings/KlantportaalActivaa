...
import { RefreshCw, Search, Save, Trash2, Filter, Check, X } from "lucide-react";
...
export default function AbonnementenOverzicht() {
  ...
  if (rijen === null) return <div style={{ fontSize: 12.5, color: KLEUR.mutedTekst }}>Abonnementen ophalen…</div>;
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap", marginBottom: 10 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 15, fontWeight: 700 }}>
            <RefreshCw size={17} color={KLEUR.blauw} /> Abonnementen op vragenlijsten
          </div>
          <div style={{ fontSize: 13, color: KLEUR.subtekst, marginTop: 4, maxWidth: 760 }}>
            Alle lopende (en gestopte) abonnementen op vaste uitvragen, over alle cliënten. Zoek, filter en
            bewaar je favoriete filtercombinaties.
          </div>
        </div>
      ...
      <div style={{ fontSize: 12, color: KLEUR.mutedTekst, marginBottom: 6 }}>{gefilterd.length} abonnement{gefilterd.length === 1 ? "" : "en"}{gefilterd.length !== zichtbaar.length ? ` · ${zichtbaar.length} getoond` : ""}</div>
      ...
      <div style={{ overflowX: "auto", border: `1px solid ${KLEUR.rand}`, borderRadius: 10 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900 }}>
          ...
          <tbody>
            {zichtbaar.length === 0 ? (
              <tr><td style={{ ...td, color: KLEUR.mutedTekst }} colSpan={10}>Geen abonnementen die aan de filters voldoen.</td></tr>
            ) : zichtbaar.map((r) => (
              <tr key={r.accountId + r.lijstId}>
                <td style={td}>
                ...
                <td style={td}>{r.eerstvolgende || <span style={{ color: KLEUR.mutedTekst }}>—</span>}</td>
                <td style={td}>{r.actief ? badge("Actief", KLEUR.groen, "#E7F2EA") : badge("Gestopt", KLEUR.mutedTekst, "#F0F0EC")}</td>
              </tr>
            ))}
          </tbody>
        </table>
